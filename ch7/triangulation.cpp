...
void find_feature_matches(const Mat &img_1, const Mat &img_2,
  ...
  // used in OpenCV3
  Ptr<FeatureDetector> detector = ORB::create(); // Ptr,聪明在什么地方?就是可以自动销毁
  Ptr<DescriptorExtractor> descriptor = ORB::create(); // Ptr是opencv的智能指针
...
void triangulation(
  ...
  // 两个相机位姿和特征点在两个相机坐标系下的归一化坐标，输出三角化后的特征点的3D坐标。
  // 注意：输出的3D坐标是齐次坐标，共四个维度，因此需要将前三个维度除以第四个维度
  // 以得到非齐次坐标xyz。这个坐标是在相机坐标系下的坐标，以输入的两个相机位姿所
  // 在的坐标系为准。

  // 转换成非齐次坐标
  for (int i = 0; i < pts_4d.cols; i++) {
    Mat x = pts_4d.col(i);
    x /= x.at<float>(3, 0); // 归一化,但是这里的z不是1.
    Point3d p(
      x.at<float>(0, 0),
      x.at<float>(1, 0),
      x.at<float>(2, 0)
