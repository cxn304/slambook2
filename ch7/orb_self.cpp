// https://blog.csdn.net/floatinglong/article/details/116279765
// Created by xiang on 18-11-25.
// https://blog.csdn.net/qq_25458977/article/details/107497334

#include <opencv2/opencv.hpp>
#include <string>
#include <nmmintrin.h>
#include <chrono>
// SIMD指令集头文件,nmmintrin.h
using namespace std;
...
typedef vector<uint32_t> DescType; // Descriptor type自定义一个type
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors) {
  ...
  int bad_points = 0;
  for (auto &kp: keypoints) {//对每一个keypoint都做计算
    ...
    descriptors.push_back(desc); // 所以每个keypoint都有descriptor
  }

  cout << "bad/total: " << bad_points << "/" << keypoints.size() << endl;
}

// brute-force matching计算descriptor之间的距离
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
  const int d_max = 40;

  for (size_t i1 = 0; i1 < desc1.size(); ++i1) {//第一张图的描述子
    if (desc1[i1].empty()) continue;
    int i11;
    i11 = static_cast<int>(i1);
    cv::DMatch m{i11, 0, 256};
    for (size_t i2 = 0; i2 < desc2.size(); ++i2) {//第二张图的描述子,然后和第一张图的当前描述子计算距离
      if (desc2[i2].empty()) continue;
      int distance = 0;
      for (int k = 0; k < 8; k++)
      { //_mm_popcnt_u32函数用于计算32位变量中1的位数
        distance += _mm_popcnt_u32(desc1[i11][k] ^ desc2[i2][k]);
      }
      if (distance < d_max && distance < m.distance) {//找出第一张图对应第二张图的最小描述子
        m.distance = distance;
        m.trainIdx = i2;
      }
    }
    if (m.distance < d_max) {//当两个描述子距离小于40,就认为匹配上了
      matches.push_back(m);
    }
  }
}